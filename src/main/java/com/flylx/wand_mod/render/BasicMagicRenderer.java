...
public class BasicMagicRenderer extends GeoProjectilesRenderer<BasicMagic> {
    ...
    public void hook_render(BasicMagic basicMagic, float f, float g, MatrixStack matrixStack,
        ...
        double p;
        double o;
        PlayerEntity playerEntity = (PlayerEntity) basicMagic.getOwner();
        ...
        VertexConsumer vertexConsumer = vertexConsumerProvider.getBuffer(CRYSTAL_BEAM_LAYER);
...
//        }
    ...
    public static void renderCrystalBeam(float dx, float dy, float dz, float tickDelta, int age, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        ...
        for (int n = 1; n <= 8; ++n) {
            ...
            vertexConsumer.vertex(matrix4f, k* 0.2f, l* 0.2f, g).color(0, 0, 0, 255).texture(m, i).overlay(OverlayTexture.DEFAULT_UV).light(light).normal(matrix3f, 0.0f, -1.0f, 0.0f).next();
            vertexConsumer.vertex(matrix4f, o* 0.2f, p* 0.2f, g).color(0, 0, 0, 255).texture(q, i).overlay(OverlayTexture.DEFAULT_UV).light(light).normal(matrix3f, 0.0f, -1.0f, 0.0f).next();
